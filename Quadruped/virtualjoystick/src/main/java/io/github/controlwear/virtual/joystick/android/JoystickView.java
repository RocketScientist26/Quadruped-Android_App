...
public class JoystickView extends View
    ...
    */
    ...
    private static final boolean DEFAULT_RIGHT_ALIGNED = false;
    ...
    public JoystickView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mRightAligned = styledAttributes.getBoolean(R.styleable.JoystickView_JV_rightAligned, DEFAULT_RIGHT_ALIGNED);
    ...
    private void initPosition() {
        ...
        if(mRightAligned == false){
            mFixedCenterX = mCenterX = mPosX = mButtonRadius;
            mFixedCenterY = mCenterY = mPosY = mButtonRadius;//getWidth() / 2; (int) ((Math.min(w, h)) / 2 * mButtonSizeRatio) //return Math.round((mPosX-mButtonRadius)*100.0f/(getWidth()-mButtonRadius*2));
        }
        else{
            //TBD
            mFixedCenterX = mCenterX = mPosX = getWidth() - mButtonRadius;
            mFixedCenterY = mCenterY = mPosY = mButtonRadius;
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        double abs = (Math.sqrt((mPosX - mCenterX) * (mPosX - mCenterX) + (mPosY - mCenterY) * (mPosY - mCenterY)))/1.62f;
    ...
    private int getStrength() {
        return (int) ((100 * Math.sqrt((mPosX - mCenterX)
                ...
                * (mPosY - mCenterY)) / mBorderRadius)/2);
    }
